        ...
        children = [
            ...
            dash_table.DataTable(
                ...
                data=pd.DataFrame(auto_evals,
                        columns=ALPHABETS[:params['n-clms']]).to_dict(),
...
def day_and_night(timestamps):
    ...
    for date in dates:
        ...
        # Chat solution:
        for date in dates:
            start = pd.to_datetime(f"{date} 07:00")
            end = pd.to_datetime(f"{date} 19:00")

            mask = (timestamps.index >= start) & (timestamps.index <= end)
            time_slice = timestamps.loc[mask]

            if len(time_slice) == 0:
                continue

            morning_idx = time_slice.iloc[0, 0]
            night_idx = time_slice.iloc[-1, 0]
